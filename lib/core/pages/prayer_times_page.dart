...
class _PrayerTimesPageState extends State<PrayerTimesPage> {
  ...
  Database? _database;
  List<Map<String, dynamic>> _next5Days = [];
  bool isDataFromCache = false;
  bool locationPermissionGranted = false;
  bool _prayerPageInitialized = false;
  ...
  Future<void> _initOnce() async {
    if (_prayerPageInitialized) return;
    _prayerPageInitialized = true;
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // KONUM KARŞILAŞTIRMA: 0.01 derece ≈ 1 km — farklı şehirler için yetmez.
  // Şehir değişikliği tespiti için 0.1 derece (~10 km) eşiği kullanıyoruz.
  // ─────────────────────────────────────────────────────────────────────────
  static const double _locationChangeThreshold = 0.1; // ~10 km

  bool _isSignificantLocationChange(
    double oldLat,
    double oldLon,
    double newLat,
    double newLon,
  ) {
    return (oldLat - newLat).abs() > _locationChangeThreshold ||
        (oldLon - newLon).abs() > _locationChangeThreshold;
  }
  ...
  Future<void> _loadPrayerTimesFromDB() async {
    try {
      ...
      print('🔍 Locations sorgulanıyor...');
      final locations = await dbSafe.query(
        'locations',
        orderBy: 'updated_at DESC',
        limit: 1,
      );
      print('🔍 Locations sayısı: ${locations.length}');
      ...
      final savedLat = _toDouble(location['latitude']);
      final savedLon = _toDouble(location['longitude']);
      final locName = (location['location_name'] as String?)?.trim();

      print('🔍 locationId: $locationId, lat: $savedLat, lon: $savedLon');

      if (locationId == null || savedLat == null || savedLon == null) {
        _getCurrentLocation();
        return;
      }
      ...
      print('🔍 Cache sorgulanıyor, locationId: $locationId, today: $today');

      final cachedTimes = await dbSafe.query(
        'prayer_times',
        where: 'location_id = ? AND date = ?',
        whereArgs: [locationId, today],
        limit: 1,
      );
      print('🔍 Cache sonuç sayısı: ${cachedTimes.length}');

      if (cachedTimes.isNotEmpty) {
        print('✅ Cache bulundu, yükleniyor...');
        final times = cachedTimes.first;
        ...
        final next5 = await getNext5DaysPrayers();
        if (mounted) setState(() => _next5Days = next5);
      } else {
        print(
          '⚠️ Cache boş! DB\'de bugüne ait veri yok, API\'den çekiliyor...',
        );
        if (mounted) setState(() => isLoading = false);
        // Cache yoksa yeni konum al ve veri çek
        _getCurrentLocation();
      }

      // Arka planda konum kontrolü
      _checkCurrentLocationAndRefreshIfNeeded(
        savedLat: savedLat,
        savedLon: savedLon,
  ...
  /// Mevcut GPS konumunu alır. Kaydedilen konumdan önemli ölçüde
  /// farklıysa (şehir değişikliği) tüm veriyi yeniden çeker.
  Future<void> _checkCurrentLocationAndRefreshIfNeeded({
    ...
    try {
      ...
      final locationChanged = _isSignificantLocationChange(
        savedLat,
        savedLon,
        position.latitude,
        position.longitude,
      ...
      if (locationChanged) {
        print(
          '📍 Konum değişti! Eski: ($savedLat,$savedLon) → '
          'Yeni: (${position.latitude},${position.longitude})',
        );
        if (locationChanged) {
          setState(() {
            isLoading = true;
            currentLocation = 'Yeni konum alınıyor...';
          });
          await _getPlaceNameAndSave(position.latitude, position.longitude);
        }
      } else {
        // Konum aynı, sadece eski veri güncelleme kontrolü yap
        print('📍 Konum değişmedi, arka plan güncelleme kontrolü yapılıyor');
        _updatePrayerTimesInBackground(
          lat: savedLat,
          lon: savedLon,
          locationId: savedLocationId,
        );
        // Bildirim de güncelle
        if (prayerTimes != null) {
      ...
      print('⚠️ Konum kontrol hatası (sessiz): $e');
      // Hata olursa mevcut cache ile devam et
      if (prayerTimes != null) {
        await _applyNotificationScheduleIfEnabled(
          prayerTimes!.map((k, v) => MapEntry(k, v.toString())),
        );
  ...
  Future<void> _updatePrayerTimesInBackground({
    required double lat,
    required double lon,
    ...
    try {
      ...
      final updatedAtStr = locations.first['updated_at'] as String?;
  ...
  void _startTimer() {
    _timer = Timer.periodic(const Duration(seconds: 1), (timer) {
      if (!mounted) return;
      // Sadece gerekli state'leri güncelle, hepsini setState'e koyma
      _updateCurrentTime();
      _calculateNextPrayer();
    });

    // Tarih güncelleme ayrı, dakikada bir yeterli
  ...
  void _updateCurrentDate() {
    final now = DateTime.now();
    final newDate = DateFormat('dd MMMM yyyy EEEE', 'en_US').format(now);
  ...
  /// Manuel "Konumu Güncelle" butonu için — tüm DB verisini temizler ve yeniden çeker.
  ...
  Future<void> _getPlaceNameAndSave(double lat, double lng) async {
    try {
      ...
      // ── Eski konumla aynı lokasyona ait tüm prayer_times'ı temizle ──
      // (yeni şehirde eski şehrin vakitlerinin kalmaması için)
      await _clearOldLocationData(lat, lng);
  ...
  String _getCurrentPrayerKey() {
    ...
    return map[nextPrayerName] ?? 'Fajr';
  }

  /// Yeni konum önemli ölçüde farklıysa eski konum kayıtlarını ve
  /// o konuma ait namaz vakitlerini DB'den siler.
  Future<void> _clearOldLocationData(double newLat, double newLon) async {
    final db = _database;
    if (db == null) return;
    try {
      final allLocations = await db.query('locations');
      for (final loc in allLocations) {
        final oldLat = _toDouble(loc['latitude']);
        final oldLon = _toDouble(loc['longitude']);
        final oldId = _toInt(loc['id']);
        if (oldLat == null || oldLon == null || oldId == null) continue;

        if (_isSignificantLocationChange(oldLat, oldLon, newLat, newLon)) {
          await db.delete(
            'prayer_times',
            where: 'location_id = ?',
            whereArgs: [oldId],
          );
          await db.delete('locations', where: 'id = ?', whereArgs: [oldId]);
          print('🗑️ Eski konum temizlendi: id=$oldId ($oldLat,$oldLon)');
        }
      }
    } catch (e) {
      print('❌ Eski konum temizleme hatası: $e');
    }
  }
  ...
  Future<int> _saveLocationToDB(
    ...
    try {
      ...
      // Eşik: 0.01 derece (~1 km) — aynı şehir içi harekette üzerine yaz
  ...
  Future<List<Map<String, dynamic>>> getNext5DaysPrayers() async {
    ...
    return await db.rawQuery(
      'SELECT * FROM prayer_times WHERE date > ? ORDER BY date ASC LIMIT 5',
      [today],
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
                  ...
                  children: [
                    ...
                    TextButton(
                      onPressed: () async {
                        ...
                        if (locationPermissionGranted) _getCurrentLocation();
                      },
                      child: const Text(
                        "İzin Ver",
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () {
          final todayTimes = prayerTimes?.map(
